...
const CustomSelect = ({ value, onChange, options, placeholder = '', disabled = false }) => {
  ...
  useEffect(() => {
    const handler = (e) => { if (ref.current && !ref.current.contains(e.target)) setOpenMenu(false); };
...
const RegionSelect = ({ valueSido, valueGungu, onChangeSido, onChangeGungu }) => {
  const gunguOptions = useMemo(() => regionMap[valueSido] || [], [valueSido]);

  useEffect(() => { onChangeGungu(''); }, [valueSido]);

  return (
    <div className="form-row grid-2">
      <CustomSelect value={valueSido} onChange={onChangeSido} placeholder="시 · 도" options={Object.keys(regionMap)} />
      <CustomSelect value={valueGungu} onChange={onChangeGungu} placeholder="구 · 군" options={gunguOptions} disabled={!valueSido} />
    </div>
  );
};
